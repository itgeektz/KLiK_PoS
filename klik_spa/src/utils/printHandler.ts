...
export function handlePrintInvoice(invoiceData: Invoice | null, options: PrintOptions = {}) {
  ...
  const executePrint = (printElement: Element) => {
  ...
  const findReadyPreviewAndPrint = (attempt = 0) => {
    ...
    const hasPreviewContent = Boolean(printContent?.innerHTML?.trim());

    if (printElement && hasPreviewContent) {
      executePrint(printElement);
      return;
    }

    if (attempt < 12) {
      setTimeout(() => findReadyPreviewAndPrint(attempt + 1), 100);
      return;
    }
